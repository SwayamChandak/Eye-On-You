...
async function loadStats() {
  ...
  const entries = Object.entries(res.timeByHost || {}).sort((a, b) => b[1] - a[1]);
...
clearBtn.addEventListener('click', () => tryAuthAnd(async () => {
  const { credentials } = await chrome.storage.local.get('credentials');
  const username = usernameEl.value.trim();
  const password = passwordEl.value;
  const hash = await sha256Base64(password + credentials.salt);
  const res = await chrome.runtime.sendMessage({ type: 'clearStats', username, passwordHash: hash });
  if (!res?.ok) {
...
async function tryAuthAnd(action) {
  ...
  // Fetch salt
  const credsSet = await chrome.runtime.sendMessage({ type: 'getCredentialsSet' });
  if (!credsSet.set) {
  ...
  const { credentials } = await chrome.storage.local.get('credentials');
  const hash = await sha256Base64(password + credentials.salt);
  const verify = await chrome.runtime.sendMessage({ type: 'verifyCredentials', username, passwordHash: hash });
  if (!verify.ok) {
...
lockBtn.addEventListener('click', () => tryAuthAnd(async () => {
  await chrome.runtime.sendMessage({ type: 'setTrackingEnabled', enabled: false });
}));

unlockBtn.addEventListener('click', () => tryAuthAnd(async () => {
  await chrome.runtime.sendMessage({ type: 'setTrackingEnabled', enabled: true });
}));
